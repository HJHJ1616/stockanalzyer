...
st.title("🚀 Quant Dashboard (V47. Fixed)")
...
default_data = pd.DataFrame([
    ...
    {"Market": "🇺🇸 Coin", "Ticker": "BTC-USD", "Date": datetime(2024, 1, 10), "Price": 45000.0, "Qty": 0.015}, # 소수점 예시
])

# 🛠️ Qty 컬럼의 step과 format을 수정하여 소수점 입력 가능하게 함
edited_df = st.sidebar.data_editor(
    ...
    column_config={
        ...
        "Price": st.column_config.NumberColumn("Price (Local)", min_value=0.01, format="%.2f"),
        "Qty": st.column_config.NumberColumn("Qty", min_value=0.000001, step=0.000001, format="%.6f"), # 소수점 6자리 허용
    },
    hide_index=True
)
...
if st.button("🤖 전체 포트폴리오 진단"):
    if not api_key: st.error("❌ API Key를 설정해주세요.")
        ...
        try:
            ...
            prompt = f"다음 포트폴리오 진단해줘:\n{summary}"
            result = safe_generate_content(prompt)
            status.empty(); st.success("진단 완료!"); st.markdown(result)
        except Exception as e:
            status.empty(); st.error(f"AI 에러: {str(e)}")
